...
open class Highlightr
{
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        #if SWIFT_PACKAGE
        // DIFFT PATCH — see Vendor/Highlightr/PATCH.md
        //
        // SwiftPM's generated Bundle.module checks only two paths: the app
        // bundle's *root* (never Contents/Resources, where a macOS app must
        // keep its resources) and an absolute path into the build directory
        // of whoever compiled it. In a shipped .app neither exists, so
        // Bundle.module hits its fatalError and the app dies at launch — but
        // only on a machine other than the one that built it.
        //
        // Look in Contents/Resources first, and fall back to Bundle.module so
        // `swift run` and tests keep working unchanged.
        let bundle = Bundle.main.resourceURL
            .map { $0.appendingPathComponent("Highlightr_Highlightr.bundle") }
